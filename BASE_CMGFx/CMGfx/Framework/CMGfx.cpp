...
BOOL CCMGfxApp::InitInstance()
{
	...
	// TO DO: You should modify this string to be something appropriate
...
{
    // free memory for singletons
    CViewport*      vp = NULL; vp = Viewport();      if ( NULL != vp ) delete vp;
    CStateManager*  sm = NULL; sm = StateManager();  if ( NULL != sm ) delete sm;
    CRasterizer*    rs = NULL; rs = Rasterizer();    if ( NULL != rs ) delete rs;
    CPrimManager*   pm = NULL; pm = PrimManager();   if ( NULL != pm ) delete pm;
    CMatrixManager* mm = NULL; mm = MatrixManager(); if ( NULL != mm ) delete mm;
    CGeoRouter*     gr = NULL; gr = GeoRouter();     if ( NULL != gr ) delete gr;
    CClipper*       cl = NULL; cl = Clipper();       if ( NULL != cl ) delete cl;
    CCamera*        cm = NULL; cm = Camera();        if ( NULL != cm ) delete cm;
}
